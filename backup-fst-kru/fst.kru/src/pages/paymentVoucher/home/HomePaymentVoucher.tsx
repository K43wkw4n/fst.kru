...
const HomePaymentVoucher = () => {
  ...
  useEffect(() => {}, [paymentVouchersByBranch]);
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (e: any) => (
        <MyRemovePop
          functionRemove={() =>
            removePaymentVoucher(e.id).then(() => {
              getPaymentVouchersByBranch(Number(currentBranchId));
            })
          }
        />
  ...
  const generatePDF = () => {
    ...
    html2pdf()
      .from(element)
      .save(evenConfirm?.paymentVoucherImportantId + ".pdf");
  };

  const dataParcel =
    evenConfirm.length !== 0 &&
    evenConfirm?.paymentVoucherItem.map((item: any, i: number) => {
      return {
        ...item.parcel,
        i: i + 1,
