...
namespace api.Service.BranchS
{
    public class BranchService : IBranchService
    {
        ...
        // หน้า report หรือหน้า dashboard 
        public async Task<object> GetCountAdminByIdAsync(int branchId)
        {
            var userCount = await _context.PersonnelinBranchs.Where(x => x.BranchId == branchId && x.Hidden.Equals(false)).CountAsync()
                + await _context.Students.Where(x => x.BranchId == branchId && !x.Hidden).CountAsync();

            var jobHistoryCount = await _context.JobHistories
                 .Include(x => x.Student)
                 .Where(x => x.Student.BranchId.Equals(branchId) && !x.Student.Hidden).CountAsync();

            var researchCount = await _context.ResearchAndProjects
                .Include(x => x.Personnel)
                    .ThenInclude(x => x.PersonnelinBranchs)
                .Where(x => x.IsUsed.Equals(true)
                && !x.Hidden
                && x.Personnel.PersonnelinBranchs
                .Any(x => x.BranchId.Equals(branchId) && !x.Hidden && x.IsUsed == 1)).CountAsync();
            ...
            var newsCount = await _context.News 
                .Where(x => x.BranchId.Equals(branchId) && !x.Hidden).CountAsync();

            return new { userCount, jobHistoryCount, researchCount,  projectCount, newsCount };
        }

        public async Task<List<Branch>> GetBranchAsync()
        {
            return await _context.Branchs.Where(x => x.Hidden == false).ToListAsync();
        }
        ...
        public async Task<object> GetBranchByIdAsync(int branchId)
        {
            ...
            return new
            {
                ...
                SlideShow = result.SlideShows.Where(x => !x.Hidden).Select(x => new
                {
                    x.ID,
                    x.SlideShowName,
                    x.ImageName,
        ...
        public async Task<object> CreateUpdateBranchAsync(BranchDto request)
        {
            ...
            else
            {
                ...
                result.CategoryBranch = request.CategoryBranch;
                result.CategoryMajor = request.CategoryMajor;
        ...
        public async Task<object> GetSlideShowByIdAsync(int branchId)
        {
            return await _context.SlideShows
                .Where(x => x.BranchId == branchId && !x.Hidden).ToListAsync();
        }
        ...
        public async Task<object> GetNewsByIdAync(int branchId)
        {
            var result = await _context.News
                ...
                .Where(x => x.BranchId.Equals(branchId) && x.Hidden == false && x.IsUsed)
        ...
        public async Task<object> GetNewsAdminByIdAync(int branchId)
        { 
            var result = await _context.News
                .Include(x => x.NewsPhotos)
                .Where(x => x.BranchId.Equals(branchId) && x.Hidden == false)
                .OrderByDescending(x => x.CreatedAt)
                .ToListAsync();
        ...
        public async Task<object> RemoveNewsPhotoAsync(int newsPhotoId)
        {
            ...
            _context.NewsPhotos.Remove(result);
        ...
        public async Task<object> GetWeLearnAsync(int branchId)
            => await _context.WeLearns.Where(x => x.BranchId.Equals(branchId) && x.Hidden == false).ToListAsync();
        ...
        public async Task<object> GetCurriculumByIdAsync(int branchId)
        {
            var result = await _context.Curriculums
                ...
                .Include(x => x.GeneralTopics.Where(x=>x.IsUsed == 1)).ThenInclude(x => x.SubGeneralTopics)
                .Where(x => x.BranchId.Equals(branchId) && x.Hidden == false).ToListAsync();
        ...
        public async Task<object> GetCurriculumByIdAdminAsync(int branchId)
        {
            var result = await _context.Curriculums
                .Include(x => x.SubjectGroups).ThenInclude(x => x.SubSubjectGroups)
        ...
        public async Task<object> UploadFileAsync(IFormFile file) => await ProcessFileUpload(file, "upload-file-curriculum");
        ...
        public async Task<object> RemoveCurriculumAsync(int curriculumId)
        {
            ...
            //{
            //    await _uploadFileService.DeleteFileImage(result.File, "upload-file-curriculum");
            //}
        ...
        public async Task<object> GetGerneralAsync()
            => await _context.Generals.ToListAsync();

        //public async Task<object> CreateUpdateGeneralAsync()
        //{
        ...
        public async Task<object> GetSectionAdminAsync()
        ...
        public async Task<object> GetSectionAsync()
            => await _context.Sections
                    .Include(x => x.Subsections)
                    //.OrderByDescending(x => x.CreatedAt)
                    .Where(x => x.IsUsed.Equals(true) && x.Hidden.Equals(false)).ToListAsync();

        public async Task<object> CreateUpdateSectionAsync(SectionDto request)
        {
            var result = await _context.Sections
                .Include(x => x.Subsections).FirstOrDefaultAsync(x => x.ID.Equals(request.ID));
            ...
            {
                _mapper.Map(request, result);
                _context.Sections.Update(result);
            }

            return await _context.SaveChangesAsync() > 0 ? null! : 0;
        }

        public async Task<object> RemoveSectionAsync(int sectionId)
        {
            var result = await _context.Sections
                .Include(x => x.Subsections).FirstOrDefaultAsync(x => x.ID.Equals(sectionId));

            if (result is null)
            {
                return 1;
            }
            else if (result.Subsections.Count() > 0)
            {
                result.Hidden = !result.Hidden;
            }
            else
            {
                _context.Sections.Remove(result);
            }

            return await _context.SaveChangesAsync() > 0 ? null! : 0;
        }

        public async Task<object> RemoveSubsectionAsync(int subsectionId)
        {
            var result = await _context.SubSections.FirstOrDefaultAsync(x => x.ID.Equals(subsectionId));

            if (result is null) return 1;

            if (result.PDF is not null)
            {
                await _uploadFileService.DeleteFileImage(result.PDF, "upload-file-section");
            }

            _context.SubSections.Remove(result);

            return await _context.SaveChangesAsync() > 0 ? null! : 0;
        }

        public async Task<object> UploadFileSectionAsync(IFormFile file) => await ProcessFileUpload(file, "upload-file-section");

        public async Task<object> IsUsedSectionAsync(int sectionId)
        {
            var result = await _context.Sections.FirstOrDefaultAsync(x => x.ID.Equals(sectionId));
