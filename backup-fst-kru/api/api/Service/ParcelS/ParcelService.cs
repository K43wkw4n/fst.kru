...
namespace api.Service.ParcelS
{
    public class ParcelService : IParcelService
    {
        ...
        {
            return await _context.Parcels.Where(x=>x.Hidden == false).ToListAsync();
        }
        ...
        public async Task<bool> CheckAlreadyParcelAsync(ParcelRequest request)
        {
            var result = await _context.Parcels.FirstOrDefaultAsync(x => x.ParcelName == request.ParcelName
            && x.Classifier == request.Classifier && x.Price == request.Price);
