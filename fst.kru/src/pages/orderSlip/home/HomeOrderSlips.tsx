...
const HomeOrderSlips = () => {
  ...
  const columns: any = [
    ...
    {
      id: 8,
      title: "วันที่สร้าง (วัน / เดือน / ปี)",
      dataIndex: "createdAt",
      key: "createdAt",
      align: "center",
    },
  ...
  const data = orderSlipsByBranch.map((item: any, i) => {
    return {
      i: i + 1,
      id: item.id,
      ...
      // createdAt: moment(item.createdAt).add(+543, "year").format("DD/MM/YYYY"),
      budget: item.budget,
  ...
  const generatePDF = () => {
    ...
    html2pdf()
      .from(element)
      .save(evenConfirm?.orderSlipImportantId + ".pdf");
  };

  console.log("evenConfirm", evenConfirm?.orderSlipImportantId);

  const dataParcel =
    evenConfirm.length !== 0 &&
    evenConfirm?.orderSlipItem.map((item: any, i: number) => {
      return {
        ...item.parcel,
        i: i + 1,
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "center", margin: 20 }}>
        <MyTitleAdmin name={"นำเข้าใบเบิก"} />
      </div>
      ...
      <DataTable data={data} columns={columns} sizeX={2500} />
      {/* <Table
        columns={columns}
        scroll={{ x: 2500, y: 500 }}
        dataSource={data}
