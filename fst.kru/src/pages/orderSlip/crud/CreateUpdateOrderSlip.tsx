...
const CreateUpdateOrderSlip = () => {
  ...
  const dataFormat = orderSlipItem.map((item: any) => ({
    ...
    year: dayjs(item.parcel.year).format("YYYY"),
    quantity: item.quantity,
    orderSlipItemId: item.id,
  }));

  // console.log("orderSlipItem", orderSlipItem[0]);
    ...
    if (findBudget) {
      ...
      createOrderSlip({
        ...
        if (res === undefined) {
          ...
          setCurrent(+1);
          setNewOrderSlipId(res.orderSlipId);
        }
      });
    } else {
      console.log("Failed");
  ...
  const optionsBudgets = budgets.map((item, i) => {
    return {
      ...
      //             });
      //             setBudget(null);
      //             // showRemoveConfirm(item.budgetName, item.id)
      //           }}
      //         />
      //       </div>
      //     </Row>
  ...
  const columns: any = [
    ...
    },
    {
      ...
      // render: (e: any) => (
      //   <div>
      //     {e && e.year ? dayjs(e.year).add(+543, "year").format("YYYY") : null}
      //   </div>
      // ),
      width: 80,
    },
    ...
    },
    {
      ...
      align: "center",
      render: (e: any) => (
        <MyRemovePop functionRemove={() => remove(e)} />
  ...
  const remove = (e: any) => {
    setParcelSelected(parcelSelected.filter((x: any) => x.id !== e.id));
    removeOrderSlipItem(e.orderSlipItemId);
    setBudget(null);
  };
                          ...
                          <Space style={{ padding: "0 8px 4px" }}>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                budget
                                  ? result === undefined
                                    ? [createBudget(budget), setBudget(null)]
                                    : alert("มีงบประมาณนี้อยู่แล้ว")
  ...
  const options = parcel.map((item: Parcels) => {
    return {
      label:
        // "หน่วย / ราคา / ชื่อ" +
        // " : " +
        // moment(item.year).format("YYYY") +
        item.classifier + " / " + item.price + " / " + item.parcelName,
      // "ปี" +
      // " / " +
      // moment(item.year).format("YYYY") +
      // " : " +
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      >
        {current > 0 && <Button onClick={prev}>ก่อนหน้า</Button>}
        {current === steps.length - 1 && (
      ...
      <Modal
        title={
          <div style={{ display: "flex" }}>
            <div style={{ fontWeight: "bold", marginRight: 5 }}>เลือกพัสดุ</div>
            <div>(หน่วย/ราคา/ชื่อ)</div>
          </div>
        }
        centered
        open={modalOpen}
      ...
      >
        <Space direction="vertical" style={{ width: "100%", height: 480 }}>
...
export default observer(CreateUpdateOrderSlip);
