...
const CreateUpdatePaymentVoucher = () => {
  ...
  const showRemoveItemConfirm = (
    ...
    confirm({
      ...
      onOk() {
        ...
        setParcelSelected(
          parcelSelected.filter((x: any) => x.id !== parcelOfBranchId)
        );
  ...
  const onSubmitItem = () => {
    if (paymentVoucherItems.length !== 0) {
      ...
      // }).then(() => {
      navigate(RoutePath.paymentvoucher);
      // });
    } else {
      notifyNoParcel();
    }
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (e: any) => (
        ...
        //       showRemoveItemConfirm(e.parcelName, e.paymentVoucherItemId, e.id);
        //     }}
        //   >
        //     ลบ
        //   </Button>
  ...
  const remove = (e: any) => {
    setParcelSelected(
      parcelSelected.filter((x: any) => x.id !== e.parcelOfBranchId)
    );
    removePaymentVoucherItem(e.paymentVoucherItemId);
  };
      ...
      content: (
        <>
          <Form
            ...
            autoComplete="off"
            layout="vertical"
            style={{ marginBottom: 100, paddingLeft: 20, paddingRight: 20 }}
          >
            <div
  ...
  const selectProps = {
    ...
    onSelect: (_: any, option: any) => {
      ...
      const selected: any = allParcel.find((x: any) => x.id === option.id);
